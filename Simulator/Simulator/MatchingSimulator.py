...
# CSV 파일에서 취미와 비트값을 로드하는 함수
def load_hobbies_with_bits(filepath):
    df = pd.read_csv(filepath, encoding='cp949', header=None, names=['Hobby', 'Bit'])
    return df.set_index('Hobby')['Bit'].to_dict()

# 사용자 데이터셋 생성 함수
def generate_user_dataset(hobbies, num_users=10000, avg_hobbies=4):
    users_dataset = {}
    hobby_list = list(hobbies.keys())

    for user_id in range(1, num_users + 1):
        num_hobbies = np.random.poisson(avg_hobbies)
        num_hobbies = max(1, min(num_hobbies, 10))
        user_hobbies = random.sample(hobby_list, num_hobbies)
        users_dataset[user_id] = user_hobbies
